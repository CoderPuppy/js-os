	...
	var AutoCompleter = exports.AutoCompleter = (function() {
		...
		AutoCompleter.prototype.autoComplete = function autoComplete(cmdLine, position, terminal) {
			...
			if(autoCompleteFile) {
				...
				var cmdSplit = cmdName.split('/');
				return terminal.getPath(terminal.fs.getFile(cmdSplit.slice(0, cmdSplit.length - 1).join('/'), terminal.currentDir))
					.filter(function(file) {
						return file.executable && file.name.substr(0, cmdSplit[cmdSplit.length - 1].length) == cmdSplit[cmdSplit.length - 1];
					}).map(function(file) {
						return {
							name: ( cmdSplit.length > 1 ? terminal.fs.pathTo(file) : file.name ),
							full: ( cmdSplit.length > 1 ? terminal.fs.pathTo(file) : file.name ) + cmdLine.substr(cmdName.end)
						};
					});
			}
		};

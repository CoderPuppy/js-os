define(['require', 'exports', './cli-parser/js/parser', './stream', './events', './pipeSeq'], function(require, exports, cliParser, Stream, events, pipeSeq) {
	...
	var CommandRunner = exports.CommandRunner = (function() {
		...
		CommandRunner.prototype.run = function run(cmd, curDir, cb, /* internal */ sendMessage) {
			...
			function commandCallback(exitCode) {
				if(i + 1 < cmdFiles.length) {
					allowRun = true;
				
					if(cmdFiles[++i].joiner == '&&') allowRun = exitCode == 0;
					else if(cmdFiles[i].joiner == '||') allowRun = exitCode != 0;
				
					if(allowRun) {
						self.exec(cmdFiles[i], undefined, undefined, runContext, commandCallback);
					}
				} else {
					// Done
				}
...
//			return rtnCode;
	...
	var Command = exports.Command = (function() {
		function Command(cmd, fs, runContext) {
			this.cmd = cmd;
		...
		Command.prototype.find = function find() {
			...
			this.file = terminal.findCMD(this.cmdName) || {
				executable: true,
				command: {

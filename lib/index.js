define(['require', 'exports', './terminal', './env', './fs/index', './user'], function(require, exports, terminal, env, fs, user) {
	var Terminal = exports.Terminal = terminal.Terminal;
	var Environment = exports.Environment = env.Environment;
	var Filesystem = exports.Filesystem = fs.Filesystem;
	var User = exports.User = user.User;
	
	function clone(o) {
		function cloned(){}
		cloned.prototype = o;
		return new cloned;
	}
	
	exports.Machine = (function() {
		function Machine(name, fsData) {
			this.name = name;
			this.users = {};
			
			fsData = fsData || {
				root: {
					files: {
						bin: {
							files: {
								echo: {
									executable: true,
									command: {
										exec: function(args, context) {
											context.view.stream.write(args.join(' '));
											context.data.write(args.join(' '));
										}
									}
								},
								pwd: {
									executable: true,
									command: {
										exec: function(args, context) {
											context.view.stream.write(context.fs.pathTo(context.currentDir));
											context.data.write(context.fs.pathTo(context.currentDir));
										}
									}
								},
								ls: {
									executable: true,
									command: {
										exec: function(args, context) {
											var files = context.fs.listFiles(context.fs.getFile(args[0], context.currentDir));
											
											files.forEach(function(file) {
												context.view.stream.write(file.name);
											});
										}
									}
								},
								cd: {
									executable: true,
									command: {
										exec: function(args, context) {
											context.currentDir = context.fs.getFile(args[0], context.currentDir) || context.currentDir;
										}
									}
								},
								alert: {
									executable: true,
									command: {
										exec: function(args, context) {
											if(args.length > 0 && args[0]) alert(args.join(' '));
											context.data.on('data', function(d) {
												alert(d);
											}).unpause();
										}
									}
								},
								sleep: {
									executable: true,
									command: {
										exec: function(args, context, cb) {
											var time = /^[1-9]\d*$/.test(args[0]) ? parseInt(args[0]) : 1000;
											setTimeout(function() { cb(0); }, time);
										}
									}
								},
								test: {
									executable: true,
									command: {
										params: [
											{
												name: 'testParam',
												type: 'string'
											},
											{
												name: 'testNumber',
												type: 'number'
											},
											{
												name: 'testPassword',
												type: 'password'
											},
											{
												name: 'testSelection',
												type: {
													name: 'selection',
													values: {
														testValue: 'Some other value',
														thisOtherValue: 'sfj'
													}
												}
											}
										],
										exec: function(args, context) {
											console.log(arguments);
											context.data.on('data', function(d) {
												alert('someone sent me: ' + d);
											});
										}
									}
								},
								badExit: {
									executable: true,
									command: {
										exec: function(args, context) {
											context.view.stream.write('I have an error');
											context.err.write('I have an error');
											return 1;
										}
									}
								}
							}
						}
					}
				}
			};
			
			this.fs = new Filesystem(fsData);
			this.env = new Environment({
				vars: {
					PATH: "/bin"
				}
			}, this.fs);
			
			this.authMethods = clone(this.authMethods);
		}
		
		Machine.prototype.authMethods = {
			always: {
				authenticate: function authenticate(password, userPassword) {
					console.log(password, userPassword);
					
					return true;
				},
				password: function password(newP) {
					console.log(newP);
					
					return newP;
				}
			}
		};
		
		Machine.prototype.createTerminal = function createTerminal(view) {
			return new Terminal(this, view);
		};
		
		Machine.prototype.createUser = function createUser(name, auth) {
			if(this.users[name] instanceof User) throw new Error('Cannot create duplicate users');
			
			return this.users[name] = new User(this, name, auth);
		};
		
		Machine.prototype.removeUser = function removeUser(name) {
			delete this.users[name];
		};
		
		Machine.prototype.auth = function auth(method) {
			return this.authMethods[method] || this.authMethods['always'];
		};
		
		Machine.prototype.login = function login(user, pass, cb) {
			if(this.users[user]) {
				this.users[user].authenticate(pass, cb);
			}
			
			return this;
		};
		
		return Machine;
	})();
});

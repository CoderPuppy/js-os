...
//						chars.splice(startPipe - ( /^\d$/.test(chars[startPipe - 1]) ? 1 : 0), 
		...
		CommandParser.prototype.splitCommands = function splitCommand(cmdLine) {
			var curChar, splits, i, inStr, split, chars, curSplit, ori;
			...
			for(i = 0; i < cmdLine.length; i++) {
				...
				if(!inStr && (curChar == '"' || curChar == "'")) {
					...
					if(/^(?:\&|\|)$/.test(curChar) && chars[i + 1] == curChar) {
						var tmpEnd = splits.length > 1 ? splits[splits.length - 1].end : 0;
						splits.push({
							type: curChar + chars[i + 1],
							index: i,
							begin: splits.length > 1 ? tmpEnd + (ori - tmpEnd) : 0,
							end: ori + chars.slice(i + 1).join('').replace(/^(\s*)[\W\D]*/, '$1').length
						});
						i++;
					} else if(/^(?:\;|\||\n)$/.test(curChar)) {
						var tmpEnd = splits.length > 1 ? splits[splits.length - 1].end : 0;
						splits.push({
							...
							index: i,
							begin: splits.length > 1 ? tmpEnd + (ori - tmpEnd) : 0,
							end: i + chars.slice(i + 1).join('').replace(/^(\s*)[\W\D]*/, '$1').length
						});
					}
		...
		CommandParser.prototype.parse = function parse(cmdLine, execCmd, options) {
			...
			rtn = cmds.map(function(cmd) {
				return merge(merge(cmd, { str: cmd.str }), self.parseFileStreams(cmd.str));
			}).map(function(cmd) {
				return merge(cmd, {
					args: self.cliParser.parse(cmd.pipedStr, self.env.vars, execCmd).map(function(arg) {
						arg.begin = arg.begin + cmd.begin;
						arg.end = arg.end + cmd.begin;

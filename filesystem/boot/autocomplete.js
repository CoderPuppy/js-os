	...
	var AutoCompleter = exports.AutoCompleter = (function() {
		...
		AutoCompleter.prototype.autoComplete = function autoComplete(cmdLine, position, terminal) {
			...
			if(autoCompleteFile) {
								...
								if(typeof(completion) == 'object') {
									return {
										...
										full: completion.full || 
											( cmdLine.substr(0, autoCompleteArg.begin) + ( completion.replace || completion.name ) + cmdLine.substr(autoCompleteArg.end) )
									};
								} else {
									return {
										name: completion,
				...
				rtn.index = autoCompleteArg ? autoCompleteArg.begin : ( autoCompleteCMD ? autoCompleteCMD.begin : 0 );
			} else {
				var cmdSplit = cmdName ? cmdName.split('/') : [];
				rtn = terminal.getPath(terminal.fs.getFile(cmdSplit.slice(0, cmdSplit.length - 1).join('/'), terminal.currentDir))
					.filter(function(file) {
				...
				rtn.index = autoCompleteCMD ? autoCompleteCMD.begin : 0;
			}
